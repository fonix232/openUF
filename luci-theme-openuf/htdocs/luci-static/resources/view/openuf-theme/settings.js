...
/*
 * System > openUF Theme: the theme's own settings, kept in LuCI's config.
 *
 *   luci.openuf_theme.interfaces    Network > Interfaces design: list, settings, cards
 *   luci.openuf_theme.wireless      Network > Wireless design: list, settings, cards
 *   luci.openuf_theme.port_manager  the Port Manager on Interfaces: 1 or 0
 *   luci.main.mediaurlbase   the colour scheme: /luci-static/openuf (follow
 *                            the system), openuf-light or openuf-dark
 *
 ...
 * theme's tokens (cascade.css, "page: openuf-theme").
 */
...
const SCHEMES = {
	'/luci-static/openuf': () => schemePreview([ 'light', 'dark' ]),
	'/luci-static/openuf-light': () => schemePreview([ 'light' ]),
	'/luci-static/openuf-dark': () => schemePreview([ 'dark' ])
};
...
return view.extend({
	load() {
		return uci.load('luci').then(() => {
			...
			if (uci.get('luci', 'openuf_theme') == null)
				uci.add('luci', 'internal', 'openuf_theme');
		});
	},

	render() {
		const m = new form.Map('luci', _('openUF Theme'),
		...
		s = m.section(form.NamedSection, 'openuf_theme', 'internal', _('Network pages'));
		s.addremove = false;

		const designs = [
			[ 'list', PREVIEWS.list, _('Device list'), _('A compact row each, in aligned columns, like a list of devices and clients.') ],
			[ 'settings', PREVIEWS.settings, _('Settings list'), _('A line each with its facts as chips, like a settings list of networks and WiFi.') ],
			[ 'cards', PREVIEWS.cards, _('Device cards'), _('A card each, like a device panel: status, facts and traffic.') ]
		];
		...
		o = s.option(CardValue, 'mediaurlbase', _('Colour scheme'),
			_('The same choice as openUF, openUFLight and openUFDark under System › System › Language and Style.'));
		o.card('/luci-static/openuf', SCHEMES['/luci-static/openuf'], _('Follow system'), _('Light or dark, as the browser prefers, with a button in the top bar to choose.'));
		o.card('/luci-static/openuf-light', SCHEMES['/luci-static/openuf-light'], _('Light'), _('Light for everyone, whatever the browser prefers.'));
		o.card('/luci-static/openuf-dark', SCHEMES['/luci-static/openuf-dark'], _('Dark'), _('Dark for everyone, whatever the browser prefers.'));
