...
// openUF status (luci-app-openuf): read-only. Everything shown comes from the
// luci.openuf rpcd backend (root/usr/share/rpcd/ucode/luci.openuf); the
// settings have their own tab (settings.js).
	...
	render: function(s) {
		...
		if (!svc.installed)
			return E('div', { 'class': 'cbi-map' }, [
				E('h2', {}, _('openUF')),
				E('p', {}, _('openUF is not installed in /usr/share/openuf.'))
			]);
		...
		return E('div', { 'class': 'cbi-map' }, [
			...
			section(_('Network ownership'), [
				...
				[ _('Controller owns interfaces and SSIDs'), yesno(conf.own_config) ],
			...
			section(_('Firmware upgrades'), [
				[ _('Reinstall after a firmware upgrade'), badge(up.bootstrap && up.bootstrap_enabled && up.feed,
					_('ready'), _('not set up')) ],
