...
#
#   openuf-update                        # the latest release (sha256-checked)
#   openuf-update --ref v1.2.0           # a release tag, or a branch (source tree)
...
#      device under. state.json is never touched. That is what makes this safe on an adopted AP: nothing
...
# A release (latest, or a vX.Y.Z tag) is the dist.sh tarball with its .sha256
# beside it; anything else is a branch, fetched as a source tree.
if [ -z "$OPENUF_TARBALL_URL" ]; then
	case "$OPENUF_REF" in
		latest)
			OPENUF_TARBALL_URL="https://github.com/$OPENUF_REPO/releases/latest/download/openuf.tar.gz" ;;
		v[0-9]*)
			OPENUF_TARBALL_URL="https://github.com/$OPENUF_REPO/releases/download/$OPENUF_REF/openuf.tar.gz" ;;
		*)
			OPENUF_TARBALL_URL="https://codeload.github.com/$OPENUF_REPO/tar.gz/$OPENUF_REF" ;;
	esac
	case "$OPENUF_REF" in latest|v[0-9]*) OPENUF_SHA256_URL="$OPENUF_TARBALL_URL.sha256" ;; esac
fi
...
show_installed() {
	...
	if [ ! -f "$INSTALL_DIR/conf.lua" ]; then
		say "  openUF is not installed in $INSTALL_DIR (run install.sh first)"
		return 1
	fi
...
[ -f "$INSTALL_DIR/conf.lua" ] || die "openUF is not installed in $INSTALL_DIR -- this updates an existing install; use install.sh for a first install."
...
# ─── Source ─────────────────────────────────────────────────────────────────
# fetch <url> <dest>: every client an OpenWrt image might have.
fetch() {
	_url=$1; _dst=$2
...
if [ -n "$FROM" ]; then
	...
	if [ -n "$OPENUF_SHA256_URL" ]; then
		fetch "$OPENUF_SHA256_URL" "$WORK/src.sha256" || die "could not fetch $OPENUF_SHA256_URL"
		_want=$(awk '{print $1; exit}' "$WORK/src.sha256")
		_got=$(sha256sum "$WORK/src.tar.gz" | awk '{print $1}')
		[ -n "$_want" ] && [ "$_want" = "$_got" ] || die "sha256 mismatch for the release tarball"
		ok "sha256 $_got"
	fi
