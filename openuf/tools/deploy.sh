...
# if the AP has LuCI, luci-app-openuf; luci-theme-openuf too where the AP
...
for h in "$@"; do
	...
	theme=0
	if [ "$luci" = 1 ] && [ -n "$(newest luci-theme-openuf apk)" ]; then
		[ "${OPENUF_DEPLOY_THEME:-0}" = 1 ] && theme=1
		$SSH "root@$h" 'apk info -e luci-theme-openuf >/dev/null 2>&1' && theme=1
	fi
	files=$(newest openuf apk)
	[ "$luci" = 1 ] && files="$files $(newest luci-app-openuf apk)"
	[ "$theme" = 1 ] && files="$files $(newest luci-theme-openuf apk)"
	...
	[ "$theme" = 1 ] && names="$names luci-theme-openuf"
