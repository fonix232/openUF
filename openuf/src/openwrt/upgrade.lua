--[[
	...
	All of it is opt-in (UCI options):

	  option upgrade_mode 'owut'
	      ...
	      Refused unless /etc/init.d/openuf-bootstrap exists: a sysupgrade keeps
	      openUF's state and settings but not the package, and the bootstrap
	      reinstalls it from the feed on the new image's first boot.

	  option advertise_updates '1'
...
function M.owut_args()
	local own = {}
	for _, p in ipairs(M.OWN_PACKAGES) do
		if M._installed(p) then own[#own + 1] = p end
	end
	return #own > 0 and (" --remove " .. table.concat(own, ",")) or ""
end
...
function M.tick(now, conf)
	...
	M._exec("( owut check" .. M.owut_args() .. " > " .. M.CHECK_FILE .. " 2>&1; echo __done__ >> "
...
function M.start(conf)
	...
	if not M._exists(M.BOOTSTRAP) then
		return false, "no " .. M.BOOTSTRAP .. ": the new image would come up without openUF"
			.. " (it is part of the openuf package)"
	end
	if M._running() then return false, "owut is already running" end
	M._exec("( sleep 2; owut upgrade" .. M.owut_args() .. " > " .. M.LOG_FILE
