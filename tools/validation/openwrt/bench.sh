...
up)
	...
	;;
adopt)
	$AP sh -c "cp /opt/openuf/etc/init.d/openuf /etc/init.d/openuf && chmod +x /etc/init.d/openuf \
		&& sed -i 's/^\tl2_announce = true/\tl2_announce = false/' /opt/openuf/conf.lua \
		&& sed -i 's/^\([[:space:]]*\)bridge_rollback_timeout = [0-9]*/\1bridge_rollback_timeout = 60/' /opt/openuf/conf.lua; \
		syswrapper.sh set-inform http://$CTL_IP:8080/inform; /etc/init.d/openuf enable; /etc/init.d/openuf restart"
	i=0; mac=""
